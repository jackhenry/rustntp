...
impl NTSPacket {
    ...
    pub fn decode(buffer: &[u8]) -> Result<Self, crate::Error> {
        // Buffer must have at least 4 bytes (critical bit, record type, body length)
        if buffer.len() < 4 {
            return Err(crate::Error::NTSEstablishmentDecodeError(String::from(
        ...
        let record_type = (((first_byte & 0x7f) as u16) << 8) | (second_byte as u16);
        let body_length = ((buffer[2] as u16) << 8) | buffer[3] as u16;
        let mut body: Vec<u8> = Vec::new();
        if body_length > 0 {
            body = Vec::from(&buffer[4..]);
        }
